...
document.addEventListener('keydown',e=>{
  ...
  if(e.code==='KeyT')document.getElementById('time-sim').style.display='block';
});
...
if (mmCanvas) {
  mmCanvas.addEventListener('click', () => {
    document.getElementById('time-sim').style.display='block';
  });
  // Also support touch for mobile users tapping the minimap
...
document.getElementById('btn-sim-close').addEventListener('click', () => {
  document.getElementById('time-sim').style.display='none';
});

document.getElementById('btn-sim-apply').addEventListener('click', () => {
  const val = document.getElementById('sim-date').value;
  if(val) {
    sessionStorage.setItem('simTime', new Date(val).toISOString());
    location.reload(); // Reload to regenerate the world with the new time
  }
});
